...
- 恢复申请增加单飞保护，并在已进入 `RecoveryRequired`、但尚未收到服务端恢复快照时，
  允许基于 journal 中的原操作上下文发起第一次正式恢复申请；请求身份、事件身份和恢复
  申请/动作身份会先持久化，再发送，重试不会因为重复点击产生第二条物理动作。
- 自动化只读快照新增当前操作 attempt 和标准化阶段字段，便于本地脚本观察
  `PREPARING`、`UNLOCKING`、`WAITING_OPERATOR`、`VERIFYING`、`SAFE_FINISH`、`PAUSED`。
  已完成且已记录结果的旧操作不会继续作为当前操作暴露。
- 这些改动只属于 OnboardHmi；没有修改 ControlServer、protocol 或现场联调结论，
  也没有放宽恢复安全门禁。
