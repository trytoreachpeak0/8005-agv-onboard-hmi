...
**本票的证据是 run 35480344066，PASS，83 秒**，跑在 merge 了车载端顶端 `d68c8182` 之后的树上。
证据 `green/rig-compensate-then-reconnect-881aa990-35480344066/`（`_stage/` 下的构建日志已删）。

| 项 | 值 |
| --- | --- |
| control-server | `13a1db75c9380321c88362e33144120c6e045cec` |
| onboard-hmi | `881aa990e1ab1269975bf9f3cb2cd26a4791f6a3` |
| slots-simulator | `fb5f7c593742bf98bc3957b8729a38aad5321f28` |
| 整机已提交内存 | 起 9.61 GiB，min 9.21、峰值 11.66 GiB（59 个采样） |

九条判据 `L2-CR-00` 到 `L2-CR-08` 全 PASS，其中 `L2-CR-03`／`L2-CR-05`（重连之后会话回到 Ready）与
`L2-CR-08`（断开一次只重连一次、到末尾只剩一条连接）正是心跳节奏变化最可能影响的两条。

**为什么非要跑在合并之后的树上**（调度 2026-09-20 定，为这张票破了一次「无产品文件交集就不强制带入
顶端重跑」的常规）：本票与期间合入的 hmi#140、hmi#139 在产品文件上没有任何交集，但文件交集只是
代理指标。本票把心跳频率翻了一倍多，而 hmi#140 改的正是**接收循环在什么情况下终止**——心跳的回执
就是从那条循环回来的，单位时间里那条循环要多处理一倍多的往返。这种耦合不落在文件上，落在运行时，
而且只有真装置会把车载端 WPF 与模拟器真起起来跑完整条路径。

在此之前还跑过一遍：run 35477628971，onboard `6d8bd18e`（merge 之前的树），PASS 64 秒，峰值 6.46 GiB；
另有 run 35480006316，onboard `405e5b05`（同样是 merge 之前），PASS 64 秒。两遍都不作为本票证据，
只说明心跳改动本身在真装置上没问题。
