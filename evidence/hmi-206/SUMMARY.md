...
| `10-reverse-verification-four-tests.txt` | `b4fdebc`（补了审查可选的两格：一次握手补发两条；补发的那条服务端已入库）；脚本 `hmi206-mutations-4.sh`，同 `06` 的做法，四条用例各 10 遍 | 修后 4/4 绿 ×10；M1、M3 各 10/10 四条全红：原有两格与「补发两条」红在 `safety revision 2 / 3 has conflicting content`，「服务端已入库」一格红在快照号 `Assert.Equal() Failure: Values differ`（服务端这一代没写版本，不冲突，但号没有前进）；M2 四条全红在 `HANDSHAKE_SEQUENCE_INVALID` |
